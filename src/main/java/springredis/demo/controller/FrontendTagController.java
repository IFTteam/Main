...
public class FrontendTagController {
    ...
    public Tag saveTagWithUserAndJourney(@PathVariable("user")long userId, @RequestBody Tag tag) {
        return frontendTagService.saveTagWithUserAndJourney(userId,  tag);
    }


    // Task 5, input is audience_id, change to return Tag Object
    @GetMapping("/tags/audiences/{id}")
    public List<TagDetail> getTagAndJourneyByAudience(@PathVariable("id") Long audienceId) throws AudienceNotFoundException {
